...
export class Store {
 ...
 recover(){const now=Date.now();for(const r of this.db.prepare('SELECT id,metadata FROM runs WHERE ended IS NULL').all() as any[]){let metadata:any={};try{metadata=JSON.parse(r.metadata??'{}');}catch{}this.db.prepare('INSERT OR REPLACE INTO run_results VALUES (?,?)').run(r.id,JSON.stringify({run_id:r.id,agent_termination_reason:'infrastructure_failed',detail:'process_interrupted',agent_claimed_success:false,oracle_success:null,oracle_assessment_reason:'quiescence_unknown',completion_audio_delivered:null,metadata}));this.db.prepare('UPDATE runs SET ended=? WHERE id=?').run(now,r.id);}this.db.prepare('UPDATE sessions SET ended=? WHERE ended IS NULL').run(now);}
