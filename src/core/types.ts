...
export type Counters = {probes:number;actions:number;observations:number;llmCalls:number;inputTokens:number;outputTokens:number;recoveries:number;informativeProbes:number;evidenceAcquired:number;firstFeasibleProbe:number|null;evidenceClosureProbe:number|null;routerComparisons:number;routerDisagreements:number};
