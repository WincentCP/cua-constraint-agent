...
export const GOAL_PROMPT=`Parse a Bahasa Indonesia shopping goal. Return ONLY a strict JSON object: {"type":"READY","goal":{"revision":1,"size":"M","color":"biru","maxPriceIdr":150000,"quantity":1,"material":"katun"}} (material optional), or {"type":"ASK_CLARIFICATION","field":"size|color|maxPriceIdr|material|quantity","question":"one short Indonesian question"}, or {"type":"UNSUPPORTED","reason":"reason in Indonesian"}. Preserve unchanged fields on correction; increment revision supplied in context. Never infer missing values from product data. Require size, color, positive integer price, exactly quantity 1. Unsupported optimization, other domains, checkout, substitutions, or quantity !=1 must be rejected. Public task instruction is context; transcribed goal/correction is authoritative. No code, tools, selectors, or extra keys.`;
...
export class Ollama implements LocalModel {
 ...
 async complete(system:string,input:unknown,signal:AbortSignal){
  ...
  const response=await fetch('http://127.0.0.1:11435/api/chat',{method:'POST',headers:{'Content-Type':'application/json'},body,signal});
...
export class Planner {
 constructor(public model:LocalModel,private call:<T>(work:(signal:AbortSignal)=>Promise<T>)=>Promise<T>,private log:(type:string,payload:unknown,sensitive?:boolean)=>void){}
 private async json<T>(prompt:string,input:unknown,schema:z.ZodType<T>,extra:(v:T)=>void=()=>{}){
  ...
  for(let attempt=0;attempt<2;attempt++){
   ...
   try{const value=schema.parse(JSON.parse(result.text));extra(value);return value;}catch(e){error=e instanceof Error?e.message:'invalid_json';}
