...
import {constraints,RunError,StaleWork,type Candidate,type Check,type Counters,type Goal,type Observation,type Phase,type Reason,type RouteStep,type Condition} from '../core/types.ts';
...
export class AgentRun {
 ...
 private async loop(){
  while(!this.terminal){try{
   ...
   const annotations=await this.planner.annotate(g,o,m,eligible,this.counters);await this.checkpoint();const p=selectProbe(this.condition,eligible,annotations,m,active,g)!;const pChoice=selectProbe('P',eligible,annotations,m,active,g),bChoice=selectProbe('B1',eligible,annotations,m,active,g);
   const diverged=pChoice?.probeId!==bChoice?.probeId;this.counters.routerComparisons++;if(diverged)this.counters.routerDisagreements++;
   this.hooks.log('planner_proposal',{...this.envelope(),annotations,selected_probe:p,router_comparison:{P:pChoice?.probeId??null,B1:bChoice?.probeId??null,diverged}});
