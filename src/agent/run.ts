...
import {constraints,RunError,StaleWork,type Candidate,type Counters,type Goal,type Observation,type Phase,type Reason,type RouteStep,type Condition} from '../core/types.ts';
...
export type AgentOutcome={run_id:string;goal:Goal|null;selected_product:{key:string;title:string}|null;agent_claimed_success:boolean;agent_termination_reason:Reason;detail:string;counters:Counters;goal_intake_ms:number;task_wall_ms:number;cleanup_ms:number;public_refutations:boolean;completion_audio_delivered:boolean|null;verification_status:'PASS'|'FAIL'|'UNKNOWN'};
export type RunHooks={log:(type:string,payload:unknown,sensitive?:boolean)=>void;status:(phase:Phase,text?:string)=>void;ask:(text:string,confirmation?:string)=>void;finished:(outcome:AgentOutcome,quiescent:boolean)=>Promise<void>};
export class AgentRun {
 id=randomUUID();epoch=0;phase:Phase='STARTING';goal?:Goal;goalAcceptedAt?:number;intakeAt=Date.now();
 counters:Counters={probes:0,actions:0,observations:0,llmCalls:0,inputTokens:0,outputTokens:0,recoveries:0,informativeProbes:0,evidenceAcquired:0,firstFeasibleProbe:null};
 observer:Observer;planner:Planner;evidence=new EvidenceStore();routes=new Routes();approval=new Approval();
 held=false;terminal=false;effectDispatched=false;private aborts=new Set<AbortController>();private waiter?:()=>void;private inputTimer?:NodeJS.Timeout;private deadline?:NodeJS.Timeout;private intakeTimer:NodeJS.Timeout;private loopRunning=false;private clarificationCount=0;private terminalPromise?:Promise<void>;private approvedEffect?:CartEffect;private proposalEffect?:CartEffect;private acquired=new Set<string>();private conflictRead=false;
 done:Promise<void>;private resolveDone!:()=>void;private pendingGoalText='';
 constructor(public page:Page,private browser:Browser,private server:BrowserServer|undefined,private condition:Condition,model:LocalModel,private hooks:RunHooks,private autoApprove=false){
 ...
 async input(text:string){
  ...
  if(!this.held)this.hold();const ticket=this.epoch;
  try{this.hooks.log('transcript',{text,...this.envelope()},true);this.pendingGoalText=this.pendingGoalText?`${this.pendingGoalText}\nKlarifikasi: ${text}`:text;const result=await this.planner.parseGoal('',this.pendingGoalText,this.goal);this.check(ticket);
   ...
   this.goal=result.goal;this.pendingGoalText='';if(!this.goalAcceptedAt){this.goalAcceptedAt=Date.now();clearTimeout(this.intakeTimer);this.deadline=setTimeout(()=>{void this.finish('timeout','task_deadline');},LIMITS.taskMs);}
   this.approval.invalidate();this.proposalEffect=undefined;this.approvedEffect=undefined;this.setPhase('EXPLORING','Saya sedang memeriksa pilihan yang memenuhi syaratmu.');this.hooks.log('goal_accepted',{goal:this.goal,...this.envelope()});this.resume();
 ...
 private async dispatch(step:RouteStep,o:Observation,probe:boolean,grant?:CartEffect){
  ...
  let after=await this.observe();let status=verifyEffect(expected,after);const end=Date.now()+Math.min(LIMITS.verifyMs,this.remaining());
  while(status!=='PASS'&&Date.now()<end){await new Promise(r=>setTimeout(r,LIMITS.pollMs));after=await this.observe();status=verifyEffect(expected,after);}
  if(status!=='PASS'&&this.counters.recoveries<LIMITS.recoveries){this.counters.recoveries++;this.hooks.log('recovery',{...this.envelope(),kind:step.kind,attempt:1});after=await this.observe();status=verifyEffect(expected,after);}
  this.hooks.log('verification',{...this.envelope(),kind:step.kind,verification_status:status,action_success:actionSuccess});
  if(status!=='PASS')throw new RunError('execution_failed','postcondition_unverified');
 ...
 private async loop(){
  while(!this.terminal){try{
   ...
   const o=await this.observe(),g=this.goal,m=this.evidence.matrix(o.candidates,g);
   ...
   const selected=o.candidates.find(c=>feasible(m,c.key,g));
   ...
   if(!this.conflictRead&&o.candidates.some(c=>this.evidence.conflict(c.key,g))){this.conflictRead=true;await this.observe();continue;}
   ...
   if(!probes.eligible.length){await this.finish('insufficient_evidence','no_eligible_probe');return;}
   const annotations=await this.planner.annotate(g,o,m,probes.eligible,this.counters);await this.checkpoint();const p=selectProbe(this.condition,probes.eligible,annotations,m,o.candidates,g)!;
   this.hooks.log('planner_proposal',{...this.envelope(),annotations,selected_probe:p});
   // Replan after each route step; every forward restoration is charged.
   await this.dispatch(p.route[0],o,true);
 ...
 private async commit(effect:CartEffect){
  ...
  const status=verifyCart(current,effect.product,g,effect.price,true);this.hooks.log('final_verification',{...this.envelope(),verification_status:status});
 ...
 finish(reason:Reason,detail=''):Promise<void>{
  ...
  const outcome:AgentOutcome={run_id:this.id,selected_product:this.proposalEffect?this.observer.known.find(c=>c.key===this.proposalEffect!.product)??null:null,goal:this.goal?structuredClone(this.goal):null,agent_claimed_success:reason==='verified_complete',agent_termination_reason:reason,detail,counters:structuredClone(this.counters),goal_intake_ms:(this.goalAcceptedAt??terminalAt)-this.intakeAt,task_wall_ms:this.goalAcceptedAt?terminalAt-this.goalAcceptedAt:0,cleanup_ms:0,public_refutations:Boolean(this.goal&&this.observer.known.length===3&&this.observer.known.every(c=>refuted(matrix,c.key,this.goal!))),completion_audio_delivered:null,verification_status:reason==='verified_complete'?'PASS':'UNKNOWN'};
