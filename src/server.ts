...
wss.on('connection',(ws:WebSocket)=>{let bound:Session|undefined;const authTimer=setTimeout(()=>ws.close(),2500);
 ws.on('message',async raw=>{try{const m=JSON.parse(raw.toString());if(!bound){const s=session;if(m.type!=='AUTH'||!s||!equal(m.token??'',s.token)||s.ws)throw Error('unauthorized');bound=s;s.ws=ws;s.lastHeartbeat=Date.now();clearTimeout(authTimer);emit(s,'STATE',{state:s.state});if(s.turn)emit(s,'PREPARE_PLAY',s.turn);return;}
  ...
  if(m.type==='SPEECH_ONSET'){s.inputTurn=z.string().uuid().parse(m.turn_id);if(s.turn?.ready){emit(s,'REJECTED',{reason:'half_duplex_playing'});return;}s.pendingSpeech=true;clearTimeout(s.answerTimer);s.run?.hold();emit(s,'INPUT_HOLD');return;}
  ...
  if(m.type==='MIC_FAILED'){event(s,'researcher_intervention',{kind:'technical',incident:'microphone_permission_failed'});emit(s,'AUDIO_ERROR',{text:'Izin mikrofon belum tersedia. Gunakan Tab untuk ke tombol berhenti; minta peneliti memeriksa izin Chrome.'});return;}
 }catch(e){ws.send(JSON.stringify({type:'ERROR',payload:{text:e instanceof Error?e.message:'invalid_message'}}));}});
 ws.on('close',()=>{clearTimeout(authTimer);const s=bound;if(s&&s.state!=='CLOSED'&&s.state!=='CLOSING'){s.epoch++;state(s,'CLOSING');clearTurn(s);voice.close();void (async()=>{await s.run?.finish('infrastructure_failed','participant_disconnected');store.close(s.id);s.state='CLOSED';if(session===s)session=undefined;})();}});
});
