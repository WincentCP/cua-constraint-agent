...
export function renderWorld(w:World,path:string,query:URLSearchParams){
 ...
 if(path===''||path==='/'){
   body='<p>Pilih satu produk untuk melihat rincian. Semua harga adalah harga satuan rupiah.</p>'+s.products.map(p=>productGroup(p,`<p>Mulai dari ${rupiah(Math.min(...p.variants.map(v=>v.price)))}</p><p>Bahan: ${p.material}</p>${s.presentation==='EARLY'?p.variants.map(facts).join(''):''}<a href="${root}/product/${p.slug}">${l[0]} ${esc(p.title)}</a>`)).join('');
 }else if(path==='/cart'){
